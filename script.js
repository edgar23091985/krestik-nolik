...
Grid.prototype.getColumnValues = function (index) {
  ...
  let column = [];
  for (let i = index; i < this.cells.length; i += 3) {
...
Grid.prototype.getFirstWithTwoInARow = function (agent) {
  ...
  let sum = agent * 2;
  let freeCells = shuffleArray(this.getFreeCellIndices());
...
function cellClicked(id) {
  ...
  if (rand < 0.3) {
    ...
    document.getElementById(id).style.transform = "ratate(90deg)";
  }
...
function endGame(who) {
  if (who === player) {
    announceWinner("Поздравляем вы выиграли");
  } else if (who === computer) {
    announceWinner("Выиграл компютер");
  } else {
